...
input_file = f"{BASE_DIR}pubmed.csv"
output_file = f'{BASE_DIR}formated-pubmed.csv'

try:
    df = pd.read_csv(input_file)
    
    required_columns = ["Publication Title", 'Authors', 'Publication Year', 'DOI']
    available_columns = [col for col in required_columns if col in df.columns]

    if not available_columns:
        print("Brak wymaganych kolumn w pliku CSV.")
    else:
        df.rename(columns={'Publication Title': 'Title'}, inplace=True)
